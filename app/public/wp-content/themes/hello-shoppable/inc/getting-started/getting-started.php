<?php
/**
 * Hello Shoppable Notice Handler
 *
 ...
 */
add_action( 'admin_enqueue_scripts', 'hello_shoppable_widgets_backend_enqueue' );
function hello_shoppable_widgets_backend_enqueue()
{
    wp_enqueue_style( 'hello-shoppable-getting-started', get_template_directory_uri() . '/inc/getting-started/getting-started.css' );
    wp_register_script( 'hello-shoppable-custom-widgets', get_template_directory_uri() . '/inc/getting-started/getting-started.js', array( 'jquery' ), true );
    $translation = array(
        'btn_text' => esc_html__( 'Processing...', 'hello-shoppable' ),
        'nonce'    => wp_create_nonce( 'hello_shoppable_demo_import_nonce' ),
        'noncen'    => 'hello_shoppable_demo_import_nonce',
        'adminurl'    => esc_url( admin_url() ),
    );
    wp_localize_script( 'hello-shoppable-custom-widgets', 'hello_shoppable_adi_install', $translation );
    wp_enqueue_media();
    wp_enqueue_script( 'hello-shoppable-custom-widgets' );
}

/**
 * Class to handle notices and Advanced Demo Import
 *
 * Class Hello_Shoppable_Notice_Handler
 */
class Hello_Shoppable_Notice_Handler {
	...
	public function __construct() {
        ...
        add_action( 'admin_notices', array( $this, 'hello_shoppable_theme_info_welcome_admin_notice' ), 3 );
        add_action( 'wp_ajax_hello_shoppable_getting_started', array( $this, 'hello_shoppable_getting_started' ) );
    }

    /**
     * Display an admin notice linking to the about page
     */
    public function hello_shoppable_theme_info_welcome_admin_notice() {
        $theme = wp_get_theme();
	 ...
	 * return void
	 */
	public function hello_shoppable_getting_started() {

		check_ajax_referer( 'hello_shoppable_demo_import_nonce', 'security' );
        ...
        if ( ! current_user_can( 'install_plugins' ) ) {
            $status['errorMessage'] = __( 'Sorry, you are not allowed to install plugins on this site.', 'hello-shoppable' );
        ...
        if( $request == 5 && strpos( $slug, 'bosa-elementor' ) === false ){
            wp_send_json_error();
        }
        if( $request == 6 && strpos( $slug, 'contact' ) === false ){
            wp_send_json_error();
        }
        if( $request == 7 && strpos( $slug, 'elementskit' ) === false ){
            wp_send_json_error();
        }
        if( $request == 8 && strpos( $slug, 'woocommerce' ) === false ){
            wp_send_json_error();
        }
        ...
        if ( is_wp_error( $result ) ) {
            ...
            $status['errorMessage'] = __( 'Unable to connect to the filesystem. Please confirm your credentials.', 'hello-shoppable' );
...
new Hello_Shoppable_Notice_Handler;